...
const techStack = [
  ...
  { name: "Docker", icon: <SiDocker className="text-blue-500 text-5xl" /> },
...
const TechStack = () => {
  return (
    <div className="mt-5 mb-5 pb-10 text-center"> {/* Reduced top margin */}
      <h2 className="text-3xl sm:text-5xl lg:text-6xl mt-4 tracking-wide">
        Our <span className="bg-gradient-to-r from-blue-500 to-blue-800 text-transparent bg-clip-text text-5xl font-bold text-center mb-8">Tech Stack</span>
      </h2>
      ...
      <div className="flex flex-wrap justify-center gap-6 mt-8">
        {techStack.map((tech, index) => (
          <motion.div
            key={index}
            className="flex flex-col items-center bg-neutral-800 p-5 rounded-xl shadow-lg cursor-pointer border border-blue-500/30 hover:border-blue-500/70"
          ...
          >
            {tech.icon}
            <p className="mt-2 text-lg text-neutral-300">{tech.name}</p>
          </motion.div>
        ))}
      </div>
    </div>
