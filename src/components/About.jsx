...
const AboutUs = () => {
  return (
    <div className="relative bg-[#F7F6F3] text-neutral-900 py-24 px-6 overflow-hidden rounded-3xl">
      {/* Animated Background Elements */}
      <motion.div
        className="absolute top-0 left-0 w-full h-full bg-gradient-to-r from-blue-400/10 via-transparent to-purple-500/10 blur-3xl opacity-30 rounded-3xl"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      <div className="relative max-w-6xl mx-auto text-center">
        ...
        {/* Description */}
        <motion.p
          className="mt-6 text-lg sm:text-xl text-neutral-700 max-w-3xl mx-auto leading-relaxed"
          initial={{ opacity: 0, y: 50 }}
          animate={{ opacity: 1, y: 0 }}
      ...
      <div className="relative max-w-7xl mx-auto grid md:grid-cols-3 gap-10 mt-16">
        {[
          ...
          <motion.div
            key={index}
            className="p-8 rounded-3xl bg-neutral-200/40 backdrop-blur-lg border border-neutral-300/60 shadow-lg"
            whileHover={{ scale: 1.05 }}
            transition={{ duration: 0.3 }}
          >
            <h3 className="text-5xl font-bold text-blue-500">{item.number}</h3>
            <p className="text-neutral-800 text-lg mt-2">{item.label}</p>
          </motion.div>
        ))}
      </div>
